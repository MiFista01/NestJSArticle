...
export class UsersModule implements NestModule {
  configure(consumer: MiddlewareConsumer) {
    consumer.apply(HashPasswordMiddleware).forRoutes({ path: '/nestjs/reg', method: RequestMethod.POST });
    consumer.apply(HashPasswordMiddleware).forRoutes({ path: '/nestjs/user', method: RequestMethod.PUT });
  }
}